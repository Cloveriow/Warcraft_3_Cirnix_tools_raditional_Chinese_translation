...
namespace CirnoLib.Format.BLPLib
{
    public sealed class BlpTexture : IArrayable, IStreamable
    {
        ...
        public BlpTexture(byte[] data)
        {
            ...
            {
                if (bs.ReadInt32() != Signature) throw new NotSupportedException("Signature 值不匹配，或者不支持 BLP 版本。");
        ...
        private void InitByStream(Stream stream)
        {
            ...
            if (stream.ReadInt32() != Signature) throw new NotSupportedException("Signature 值不匹配，或者不支持 BLP 版本。");
